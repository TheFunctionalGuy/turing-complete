...
def post_process(files, raw):
    ...
    for line in lines:
        ...
        # Get tokens
        left, asm = line.split(" ; ")
        _, _, opcode = left.split(" | ")

        # Skip labels
        if opcode.strip():
            if raw:
                post_processed_lines.append(
                    " ".join(f"0x{hex}" for hex in opcode.strip().split(" "))
                )
            else:
                post_processed_lines.append(
                    " ".join(f"0x{hex}" for hex in opcode.strip().split(" "))
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Post-process assembly output from customasm tool."
    )
    parser.add_argument("-o", "--output", help="Output file")
    parser.add_argument(
        "-r", "--raw", action="store_true", help="Remove comments from output"
    )
    parser.add_argument(
        "files",
        metavar="FILE",
        nargs="*",
        help="Files to read, if empty stdin will be used instead",
    )
...
if __name__ == "__main__":
    ...
    lines = post_process(args.files, args.raw)
